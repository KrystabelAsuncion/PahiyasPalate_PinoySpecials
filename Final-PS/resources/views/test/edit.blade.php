...
<body>
    <div class="container d-flex justify-content-center">
        <div class="card p-4">
            ...
            <form action="{{ route('recipe.update',$recipe->id) }}" method="POST" class="mx-auto" style="width: 500px;">
                @csrf
                @method('PUT')
                <div class="mb-3">
                    <label for="name">Name:</label>
                    <input type="text" id="name" name="recipe_name" value="{{$recipe->recipe_name}}" class="form-control" required>
                </div>

                <div class="mb-3">
                    <label for="desc">Description:</label>
                    <input type="text" id="desc" name="recipe_description" value="{{$recipe->recipe_description}}" class="form-control" required>
                </div>
    ...
    <script>
        $(document).ready(function() {
            $("#add-steps").click(function() {
                var index = $(".steps-field").length; // Get the current number of subject fields
                var html = `
                    <div class="steps-field">
